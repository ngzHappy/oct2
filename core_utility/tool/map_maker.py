...
class Apply {
public:
    template<typename _Tuple_>
    static void apply(_Tuple_ &&/*argTuple*/) {
        std::cout<<"()"<<std::endl;
    }
};"""
    pass
...
def get_n(n):
    ...
    ans_string+="    template<typename _Tuple_>";
    ans_string+=get_endl();
    ans_string+="    static void apply(_Tuple_ &&argTuple) {";
    ans_string+=get_endl();
    #############################################################################
    ...
    for i in range(n):
        ...
        if i==0:#the first
            template_string="""{std::cout<<std::get<N_N>(std::forward<_Tuple_>(argTuple))<<",";}""";
        elif i==(n-1):#the last
            template_string="""{std::cout<<std::get<N_N>(std::forward<_Tuple_>(argTuple));}""";
        else:
            template_string="""{std::cout<<std::get<N_N>(std::forward<_Tuple_>(argTuple))<<",";}""";
